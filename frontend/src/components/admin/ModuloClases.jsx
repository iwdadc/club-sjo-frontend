...
import { getClases, editarClase } from "../../services/claseService"
...
function ModalEditarClase({ clase, profesores, onGuardar, onCerrar }) {
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center px-4 z-50">
        <div className="bg-white rounded-2xl w-full max-w-md shadow-xl overflow-hidden">
            ...
            <form onSubmit={handleSubmit(onGuardar)} className="p-6 flex flex-col gap-4">
                <div className="flex flex-col gap-1">
                    ...
                    />
                    {errors.horario && <span className="text-xs text-red-500">{errors.horario.message}</span>}
                </div>
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <select
                    {...register("profesor", { required: "El profesor es obligatorio" })}
                    className="h-10 border border-gray-200 rounded-lg px-3 text-sm focus:outline-none focus:border-[#1E3A8A] bg-gray-50"
                    >
                    ...
                    {errors.profesor && <span className="text-xs text-red-500">{errors.profesor.message}</span>}
                </div>
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <input
                    type="number"
                    {...register("cupoMax", { required: "El cupo es obligatorio", min: 1 })}
                    className="h-10 border border-gray-200 rounded-lg px-3 text-sm focus:outline-none focus:border-[#1E3A8A] bg-gray-50"
                    />
                    {errors.cupoMax && <span className="text-xs text-red-500">{errors.cupoMax.message}</span>}
                </div>
...
function ModuloClases() {
    ...
    if (cargando) return <p className="text-sm text-gray-400">Cargando clases...</p>
    ...
    return (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            ...
            <div>
                ...
                <p className="text-2xl font-semibold text-gray-900">{clases.length}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            return (
                ...
                <div className="flex justify-between items-start mb-3">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">{clase.horario}</p>
                    </div>
                ...
                <div>
                    ...
                    <span className={llena ? 'text-red-500 font-medium' : 'text-gray-400'}>
                        {llena ? 'Sin cupos' : `${cuposLibres} libre${cuposLibres !== 1 ? 's' : ''}`}
                    </span>
                    </div>
                    <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                    <div
                        className={`h-full rounded-full transition-all ${colorOcupacion(clase.inscriptos, clase.cupoMax)}`}
                        style={{ width: `${Math.min((clase.inscriptos / clase.cupoMax) * 100, 100)}%` }}
                    />
                    </div>
                </div>
